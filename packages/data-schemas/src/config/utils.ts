 ...
 * Priority:
 * 1. AGENTCHAT_LOG_DIR environment variable
 * 2. If running within AgentChat monorepo (when cwd ends with /api), use api/logs
...
export const getLogDirectory = (): string => {
  if (process.env.AGENTCHAT_LOG_DIR) {
    return process.env.AGENTCHAT_LOG_DIR;
  }
  ...
  // For AgentChat project structure, use api/logs
  // For external consumers, they should set AGENTCHAT_LOG_DIR
  if (cwd.includes('AgentChat')) {
    return apiLogsPath;
  }

...
import type { IUser, OIDCTokens } from 'data-schemas';
 ...
 */
export const GRAPH_TOKEN_PLACEHOLDER = '{{AGENTCHAT_GRAPH_ACCESS_TOKEN}}';
...
export function processOpenIDPlaceholders(
  ...
  for (const field of OPENID_TOKEN_FIELDS) {
    const placeholder = `{{AGENTCHAT_OPENID_${field}}}`;
  ...
  const genericPlaceholder = '{{AGENTCHAT_OPENID_TOKEN}}';

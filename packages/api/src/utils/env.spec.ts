...
import type { MCPOptions } from 'agentchat-data-provider';
...
describe('resolveHeaders', () => {
  ...
  it('should process full user object placeholders', () => {
    ...
    const headers = {
      ...
      'User-Name': '{{AGENTCHAT_USER_NAME}}',
      'User-Username': '{{AGENTCHAT_USER_USERNAME}}',
  ...
  it('should handle missing user fields gracefully', () => {
    ...
    const headers = {
      'User-Email': '{{AGENTCHAT_USER_EMAIL}}',
      'User-Username': '{{AGENTCHAT_USER_USERNAME}}',
      'Non-Existent': '{{AGENTCHAT_USER_NONEXISTENT}}',
    };
  ...
  it('should prioritize custom user variables over user fields', () => {
    ...
    const customUserVars = {
      AGENTCHAT_USER_EMAIL: 'custom-email@example.com',
    };

    const headers = {
      'Test-Email': '{{AGENTCHAT_USER_EMAIL}}',
    };
  ...
  it('should replace all allowed user field placeholders', () => {
    ...
    const headers = {
      'X-User-ID': '{{AGENTCHAT_USER_ID}}',
      'X-User-Name': '{{AGENTCHAT_USER_NAME}}',
      'X-User-Username': '{{AGENTCHAT_USER_USERNAME}}',
      'X-User-Email': '{{AGENTCHAT_USER_EMAIL}}',
      'X-User-Provider': '{{AGENTCHAT_USER_PROVIDER}}',
      'X-User-Role': '{{AGENTCHAT_USER_ROLE}}',
      'X-User-GoogleId': '{{AGENTCHAT_USER_GOOGLEID}}',
      'X-User-FacebookId': '{{AGENTCHAT_USER_FACEBOOKID}}',
      'X-User-OpenIdId': '{{AGENTCHAT_USER_OPENIDID}}',
      'X-User-SamlId': '{{AGENTCHAT_USER_SAMLID}}',
      'X-User-LdapId': '{{AGENTCHAT_USER_LDAPID}}',
      'X-User-GithubId': '{{AGENTCHAT_USER_GITHUBID}}',
      'X-User-DiscordId': '{{AGENTCHAT_USER_DISCORDID}}',
      'X-User-AppleId': '{{AGENTCHAT_USER_APPLEID}}',
      'X-User-EmailVerified': '{{AGENTCHAT_USER_EMAILVERIFIED}}',
      'X-User-TwoFactorEnabled': '{{AGENTCHAT_USER_TWOFACTORENABLED}}',
      'X-User-TermsAccepted': '{{AGENTCHAT_USER_TERMSACCEPTED}}',
    };
  ...
  it('should handle multiple placeholders in one value', () => {
    ...
    const headers = {
      'X-Multi': 'User: {{AGENTCHAT_USER_ID}}, Env: ${TEST_API_KEY}, Custom: {{MY_CUSTOM}}',
    };
  ...
  it('should handle a mix of all types', () => {
    ...
    const headers = {
      'X-User': '{{AGENTCHAT_USER_ID}}',
      'X-Env': '${TEST_API_KEY}',
      'X-Custom': '{{MY_CUSTOM}}',
      'X-Multi': 'ID: {{AGENTCHAT_USER_ID}}, ENV: ${TEST_API_KEY}, CUSTOM: {{MY_CUSTOM}}',
      'X-Unknown': '{{NOT_A_REAL_PLACEHOLDER}}',
      'X-Empty': '',
      'X-Boolean': '{{AGENTCHAT_USER_EMAILVERIFIED}}',
    };
    const customVars = { MY_CUSTOM: 'custom-value' };
    const result = resolveHeaders({ headers, user, customUserVars: customVars });
  ...
  it('should process AGENTCHAT_BODY placeholders', () => {
    const body = {
      conversationId: 'conv-123',
      parentMessageId: 'parent-456',
      messageId: 'msg-789',
    };
    const headers = { 'X-Conversation': '{{AGENTCHAT_BODY_CONVERSATIONID}}' };
    const result = resolveHeaders({ headers, body });
    expect(result['X-Conversation']).toBe('conv-123');
  });

  it('should not resolve env vars introduced via AGENTCHAT_BODY placeholders', () => {
    const body = {
    ...
    const headers = {
      'X-Conv': '{{AGENTCHAT_BODY_CONVERSATIONID}}',
      'X-Parent': '{{AGENTCHAT_BODY_PARENTMESSAGEID}}',
      'X-Msg': '{{AGENTCHAT_BODY_MESSAGEID}}',
    };
    const result = resolveHeaders({ headers, body });
  ...
  it('should not resolve env vars introduced via AGENTCHAT_USER placeholders', () => {
    const user = createTestUser({ name: '${TEST_API_KEY}' });
    const headers = { 'X-Name': '{{AGENTCHAT_USER_NAME}}' };
    const result = resolveHeaders({ headers, user });
  ...
  describe('non-string header values (type guard tests)', () => {
    ...
    it('should handle numeric values with placeholders', () => {
      ...
      const headers = {
        ...
        'X-String-With-Placeholder': '{{AGENTCHAT_USER_ID}}',
      };
      const result = resolveHeaders({ headers, user });
    ...
    it('should not crash when calling includes on non-string body field values', () => {
      ...
      const headers = {
        'X-Conv-Id': '{{AGENTCHAT_BODY_CONVERSATIONID}}',
        'X-Number': 999 as unknown as string,
      };
...
describe('resolveNestedObject', () => {
  ...
  it('should process placeholders in string values while preserving structure', () => {
    ...
    const obj = {
      thinking: {
        type: 'enabled',
        budget_tokens: 2000,
        user_context: '{{AGENTCHAT_USER_ID}}',
      },
      anthropic_beta: ['output-128k-2025-02-19'],
  ...
  it('should process strings in arrays', () => {
    const user = { id: 'user-123' };
    const obj = {
      headers: ['Authorization: Bearer ${TEST_API_KEY}', 'X-User-Id: {{AGENTCHAT_USER_ID}}'],
      values: [1, 2, 3],
      mixed: ['string', 42, true, '{{AGENTCHAT_USER_ID}}'],
    };
  ...
  it('should handle body placeholders in nested objects', () => {
    ...
    const obj = {
      metadata: {
        conversation: '{{AGENTCHAT_BODY_CONVERSATIONID}}',
        parent: '{{AGENTCHAT_BODY_PARENTMESSAGEID}}',
        count: 5,
      },
    };
  ...
  it('should handle arrays of objects', () => {
    ...
    const obj = {
      items: [
        { name: 'item1', user: '{{AGENTCHAT_USER_ID}}', count: 1 },
        { name: 'item2', user: '{{AGENTCHAT_USER_ID}}', count: 2 },
      ],
    };
  ...
  it('should not modify the original object', () => {
    ...
    expect(originalObj.thinking.user_id).toBe('{{AGENTCHAT_USER_ID}}');
  });
});
...
describe('processMCPEnv', () => {
  ...
  it('should process user field placeholders in all fields', () => {
    ...
    const options: MCPOptions = {
      ...
      env: {
        ...
        USER_ROLE: '{{AGENTCHAT_USER_ROLE}}',
      },
      args: ['--user', '{{AGENTCHAT_USER_USERNAME}}', '--id', '{{AGENTCHAT_USER_ID}}'],
    };
  ...
  it('should process body placeholders in all fields', () => {
    ...
    const options: MCPOptions = {
      type: 'streamable-http',
      url: 'https://api.example.com/conversations/{{AGENTCHAT_BODY_CONVERSATIONID}}',
      headers: {
        'X-Parent-Message': '{{AGENTCHAT_BODY_PARENTMESSAGEID}}',
        'X-Message-Id': '{{AGENTCHAT_BODY_MESSAGEID}}',
      },
    };
  ...
  it('should handle mixed placeholders in OAuth configuration', () => {
    ...
    const options: MCPOptions = {
      ...
      oauth: {
        ...
        client_id: '${OAUTH_CLIENT_ID}',
        client_secret: '${OAUTH_CLIENT_SECRET}',
        scope: 'user:{{AGENTCHAT_USER_ID}} conversation:{{AGENTCHAT_BODY_CONVERSATIONID}}',
        redirect_uri: 'http://localhost:3000/user/{{AGENTCHAT_USER_EMAIL}}/callback',
      },
    };
  ...
  it('should handle all placeholder types in a single value', () => {
    ...
    const options: MCPOptions = {
      ...
      env: {
        COMPLEX_VALUE:
          'User: {{AGENTCHAT_USER_ID}}, Custom: {{CUSTOM_VAR}}, Body: {{AGENTCHAT_BODY_CONVERSATIONID}}, Env: ${TEST_API_KEY}',
      },
    };
  ...
  describe('non-string values (type guard tests)', () => {
    ...
    it('should not crash with numeric body field values', () => {
      ...
      const options: MCPOptions = {
        ...
        args: [],
        env: {
          CONV_ID: '{{AGENTCHAT_BODY_CONVERSATIONID}}',
          PORT: 8080 as unknown as string,
        },
      };
  ...
  describe('dbSourced flag', () => {
    ...
    it('should NOT resolve {{AGENTCHAT_USER_*}} when dbSourced is true', () => {
      ...
      if (isStreamableHTTPOptions(result)) {
        expect(result.headers?.['X-User-Id']).toBe('{{AGENTCHAT_USER_ID}}');
        expect(result.headers?.['X-User-Email']).toBe('{{AGENTCHAT_USER_EMAIL}}');
      } else {
        throw new Error('Expected streamable-http options');
      }
    });

    it('should NOT resolve {{AGENTCHAT_OPENID_*}} when dbSourced is true', () => {
      const user = {
      ...
      const options: MCPOptions = {
        ...
        url: 'https://api.example.com',
        headers: {
          Authorization: 'Bearer {{AGENTCHAT_OPENID_ACCESS_TOKEN}}',
        },
      };
      ...
      if (isStreamableHTTPOptions(result)) {
        expect(result.headers?.Authorization).toBe('Bearer {{AGENTCHAT_OPENID_ACCESS_TOKEN}}');
      } else {
        throw new Error('Expected streamable-http options');
      }
    });

    it('should NOT resolve {{AGENTCHAT_BODY_*}} when dbSourced is true', () => {
      const body = {
        conversationId: 'conv-123',
        parentMessageId: 'parent-456',
        messageId: 'msg-789',
      ...
      const options: MCPOptions = {
        ...
        url: 'https://api.example.com',
        headers: {
          'X-Conversation': '{{AGENTCHAT_BODY_CONVERSATIONID}}',
        },
      };
      ...
      if (isStreamableHTTPOptions(result)) {
        expect(result.headers?.['X-Conversation']).toBe('{{AGENTCHAT_BODY_CONVERSATIONID}}');
      } else {
        throw new Error('Expected streamable-http options');
      }
    });
    ...
    it('should resolve customUserVars but block all other placeholders when dbSourced is true', () => {
      ...
      const options: MCPOptions = {
        ...
        headers: {
          ...
          'X-User-Id': '{{AGENTCHAT_USER_ID}}',
          'X-Body': '{{AGENTCHAT_BODY_CONVERSATIONID}}',
        },
      };
      ...
      if (isStreamableHTTPOptions(result)) {
        ...
        expect(result.headers?.['X-User-Id']).toBe('{{AGENTCHAT_USER_ID}}');
        expect(result.headers?.['X-Body']).toBe('{{AGENTCHAT_BODY_CONVERSATIONID}}');

...
import type { Logger } from 'agents';
...
describe('getBasePath', () => {
  ...
  it('should return base path for subdirectory deployment', () => {
    ...
    expect(getBasePath()).toBe('/agentchat');
  });
  ...
  it('should handle nested subdirectories', () => {
    ...
    expect(getBasePath()).toBe('/apps/agentchat');
  });
  ...
  it('should handle URLs with query parameters', () => {
    process.env.DOMAIN_CLIENT = 'http://localhost:3080/agentchat?param=value';
    expect(getBasePath()).toBe('/agentchat');
  });

  it('should handle URLs with fragments', () => {
    process.env.DOMAIN_CLIENT = 'http://localhost:3080/agentchat#section';
    expect(getBasePath()).toBe('/agentchat');
  });
  ...
  it('should handle URLs with ports', () => {
    process.env.DOMAIN_CLIENT = 'http://localhost:8080/agentchat';
    expect(getBasePath()).toBe('/agentchat');
  });

  it('should handle URLs with subdomains', () => {
    process.env.DOMAIN_CLIENT = 'https://app.example.com/agentchat';
    expect(getBasePath()).toBe('/agentchat');
  });
});

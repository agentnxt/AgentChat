...
const { genAzureEndpoint, logAxiosError } = require('api');
const { extractEnvVariable, STTProviders } = require('agentchat-data-provider');
...
class STTService {
  ...
  async getProviderSchema(req) {
    ...
    if (!sttSchema) {
      throw new Error(
        'No STT schema is set. Did you configure STT in the custom config (agentchat.yaml)?',
      );
    }
